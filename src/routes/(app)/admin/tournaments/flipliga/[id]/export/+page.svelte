	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
		<Card class="hover:shadow-lg transition-shadow">
			<div class="flex flex-col h-full">
				...
				{/if}
				<Button disabled={!pdfEnabled} on:click={() => generateResults(true)} class="w-full mt-auto text-base">
		...
		<Card class="hover:shadow-lg transition-shadow">
			<div class="flex flex-col h-full">
				...
				{#if !pdfEnabled}
					<Badge color="gray" class="mb-3">Liga nicht abgeschlossen</Badge>
				{/if}
				<Button disabled={!pdfEnabled} on:click={() => generateResults(false)} class="w-full mt-auto text-base">
					<FilePdfOutline class="w-4 h-4 mr-2" />
					Generiere PDF
				</Button>
			</div>
